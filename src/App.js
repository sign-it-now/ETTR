...
const STAGES = [
{ key:"rate_con",    label:"Rate Con Received",  icon:"📋", color:"#7c3aed",
desc:"Rate confirmation received from broker — review and upload",
...
desc:"Load dispatched to driver",
action:"Dispatch to Driver",
who:"carrier" },
{ key:"picked_up",  label:"Picked Up",            icon:"🏭", color:"#f59e0b",
desc:"Driver at shipper — get BOL and upload, then mark picked up",
action:"Mark Picked Up",
who:"driver" },
{ key:"in_transit", label:"In Transit",            icon:"🚛", color:"#f97316",
desc:"Load picked up and in transit to delivery location",
action:"Mark In Transit",
who:"driver" },
{ key:"delivered",  label:"Delivered",             icon:"📦", color:"#22c55e",
desc:"Delivered to receiver — upload signed BOL and POD",
action:"Confirm Delivery",
who:"driver" },
{ key:"invoiced",   label:"Invoiced",              icon:"📤", color:"#10b981",
desc:"Invoice sent to broker — awaiting payment",
action:"Submit Invoice to Broker",
who:"carrier" },
...
dispatched:"2026-02-09",
picked_up:"2026-02-10", in_transit:"2026-02-10",
...
{needAction.length>0&&(
<div style={{...card({borderColor:"#f59e0b",background:"#78350f10",marginBottom:16})}}>
<div style={{fontSize:12,color:"#fbbf24",fontWeight:900,marginBottom:8}}>
⚡ {needAction.length} LOAD{needAction.length>1?"S":""} WAITING FOR YOUR ACCEPTANCE
</div>
{needAction.map(l=>(
...
{/* Stats grid — 2-col on mobile, 4-col on wider */}
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:22}}>
{[
{l:"GROSS REVENUE",  v:fmt(gross),       c:C.accent,  sub:`${my.length} total loads`, p:"loads"},
{l:"OPEN LOADS",     v:open.length,      c:"#f59e0b",  sub:"In progress",              p:"loads"},
...
style={card({cursor:"pointer",borderColor:`${s.c}30`,transition:"border-color .2s",padding:16})}>
...
{/* Load cards — mobile-first */}
<div style={{marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
<div style={{fontSize:11,color:C.accent,fontWeight:900,letterSpacing:1.5}}>RECENT LOADS</div>
<button onClick={()=>setPage("loads")} style={{...btn(C.card2,"#94a3b8"),fontSize:10,padding:"5px 12px"}}>
View All →
</button>
</div>

{my.length===0&&(
<div style={card({textAlign:"center",padding:40,color:C.border})}>
No loads yet —{" "}
...
style={card({cursor:"pointer",borderColor:`${st.color}40`,transition:"border-color .2s",padding:16})}>
{/* Mini progress bar */}
<div style={{display:"flex",gap:2,marginBottom:10}}>
{STAGES.filter(s=>s.key!=="paid").map((s,i)=>(
<div key={s.key}
...
<span style={{display:"inline-block",background:`${st.color}20`,color:st.color,
border:`1px solid ${st.color}50`,borderRadius:20,
padding:"3px 10px",fontSize:10,fontWeight:800,marginBottom:8,letterSpacing:0.5}}>
{st.icon} {st.label}
</span>
...
<div style={{fontSize:20,fontWeight:900,color:C.text,flexShrink:0}}>
{fmt(load.grossRate)}
</div>
</div>
</div>
...
const [tab,setTab] = useState("details"); // details | documents | charges
...
setShowInvoice(false);
  ...
  <div style={{background:`${sc}12`,border:`2px solid ${sc}50`,
    ...
    {/* Progress track — 6 main stages (excluding paid) */}
    {(()=>{
      const timelineStages = STAGES.filter(s=>s.key!=="paid");
      const tsi = timelineStages.findIndex(s=>s.key===load.stage);
      return(
        <div style={{display:"flex",gap:0,marginBottom:16,overflowX:"auto",paddingBottom:4}}>
          {timelineStages.map((s,i)=>{
            const done = i<tsi || load.stage==="paid";
            const cur = i===tsi && load.stage!=="paid";
            return(
              <div key={s.key} style={{display:"flex",alignItems:"center",flex:1,minWidth:0}}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",flex:"0 0 auto"}}>
                  <div style={{width:30,height:30,borderRadius:"50%",
                    background:done?"#22c55e":cur?s.color:C.card2,
                    display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:done?"14px":"13px",
                    border:`2px solid ${done?"#22c55e":cur?s.color:C.border}`,
                    transition:"all .3s",flexShrink:0}}>
                    {done?"✓":s.icon}
                  </div>
                  <div style={{fontSize:8,color:done?"#22c55e":cur?s.color:C.border,
                    textAlign:"center",marginTop:3,maxWidth:56,
                    lineHeight:1.2,letterSpacing:0.3,fontWeight:cur?800:400}}>
                    {s.label}
                  </div>
                </div>
                {i<timelineStages.length-1&&(
                  <div style={{flex:1,height:2,
                    background:done?"#22c55e":C.card2,margin:"0 2px",
  ...
  <div style={{display:"flex",gap:2,marginBottom:16,
    background:C.side,borderRadius:10,padding:4,
    border:`1px solid ${C.border}`}}>
    {[{k:"details",l:"📍 Details"},{k:"documents",l:"📄 Documents"},{k:"charges",l:"💵 Charges"}].map(t=>(
      <button key={t.k} onClick={()=>setTab(t.k)}
        style={{flex:1,padding:"9px 0",borderRadius:8,border:"none",
          cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:700,
          background:tab===t.k?C.accent:"transparent",
          color:tab===t.k?"#fff":C.muted,transition:"all .15s"}}>
        {t.l}
      </button>
    ))}
  </div>
  ...
  {tab==="documents"&&(
    <div>
      {/* Stage-contextual doc uploads */}
      {(load.stage==="rate_con"||load.stage==="accepted")&&(
        <Sec t="📋 RATE CONFIRMATION" bc="#7c3aed40">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <DocBox docKey="rateCon" label="Rate Con — Original" required/>
            {load.stage==="accepted"&&(
              <DocBox docKey="rateConSigned" label="Rate Con — Signed/Accepted"/>
            )}
          </div>
          {load.stage==="rate_con"&&(
            <div style={{marginTop:14}}>
              <div style={{fontSize:11,color:C.dim,marginBottom:10}}>
                After uploading, confirm acceptance to notify broker.
              </div>
      ...
      {(load.stage==="picked_up"||load.stage==="in_transit")&&(
        <Sec t="🏭 PICKUP — BOL FROM SHIPPER" bc="#f59e0b40">
          <div style={{fontSize:11,color:C.dim,marginBottom:12}}>
            Get BOL from shipper. Upload unsigned first, then signed after shipper signs.
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      ...
      {load.stage==="delivered"&&(
        <Sec t="📦 DELIVERY DOCUMENTS" bc="#22c55e40">
          <div style={{fontSize:11,color:C.dim,marginBottom:12}}>
            Upload signed POD from receiver and any lumper receipts.
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <DocBox docKey="pod" label="POD — Proof of Delivery" required/>
            <DocBox docKey="lumper" label="Lumper / Unload Receipt"/>
          </div>
        </Sec>
      )}

      {stageIdx(load.stage)>=stageIdx("delivered")&&(
        <Sec t="🧾 BILLING PACKAGE" bc="#10b98140">
          <div style={{fontSize:11,color:C.dim,marginBottom:14}}>
            Invoice, rate con, signed BOL, POD, and all receipts — send to broker.
          </div>
              ...
              .map(d=>{
                ...
                return(
                  <div key={d.k} style={{display:"flex",alignItems:"center",gap:6,
                    ...
                    <span>{has?"✅":"⬜"}</span>
                    <span style={{fontSize:10,color:has?C.green:C.muted,fontWeight:700}}>{d.l}</span>
                  </div>
                );
              })}
          </div>
